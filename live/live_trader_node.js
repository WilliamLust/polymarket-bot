...
function normalizeCategory(raw, question, slug) {
  ...
  if (/\b(ufc|mma|boxing|wwe|wrestling|golf|tennis|f1|formula|eurovision)\b/.test(qs)) return "sports";
  if (/\b(fighter|match|game|bout|round|knockout|playoff)\b/.test(qs)) return "sports";
  if (/\b(spread|moneyline|parlay|over under|o\/u )\b/.test(qs)) return "sports";
  if (/\b(elect|election|vote|voter|ballot|campaign|primary|runoff)\b/.test(qs)) return "politics";
  if (/\b(president|senator|congress|governor|mayor|parliament|council seat|prime minister|chancellor)\b/.test(qs)) return "politics";
  if (/\b(democrat|republican|conservative|labour|tory|reform uk|liberal|gop|midterm)\b/.test(qs)) return "politics";
  if (/\b(bill|legislation|supreme court|impeach|veto|filibuster|out as president|out as prime minister)\b/.test(qs)) return "politics";
  ...
  if (/\b(gdp|inflation|federal reserve|cpi|unemployment)\b/.test(qs)) return "finance";
  if (/\b(oil|natural gas|gold|silver|commodity|crude|brent)\b/.test(qs)) return "finance";
  if (/\b(up or down|price target)\b/.test(qs)) return "finance";
  if (/\b(research|experiment|physics|chemistry|biology|genome|dna|rna|particle|quantum|lab leak|laboratory)\b/.test(qs)) return "science";
  if (/\b(climate change|emissions|carbon|biodiversity|endangered|conservation|ocean|arctic|antarctic|earthquake|volcano|tsunami|hurricane|tornado)\b/.test(qs)) return "science";
  if (/\b(pandemic|outbreak|vaccine|disease|cdc |fda|clinical trial|drug approval)\b/.test(qs)) return "health";
  if (/\b(hospital|mortality|life expectancy|obesity|mental health|overdose|flu|measles|ebola|hantavirus|confirmed case)\b/.test(qs)) return "health";
  if (/\b(invade|invasion|ceasefire|treaty|sanctions|nato|un security|united nations|diplomatic meeting|peace deal|strike on|strike iran|regime fall)\b/.test(qs)) return "world";
  if (/\b(refugee|immigration|deport|annex|territory|embargo|tariff|regime change|warships|air strike|military strike|enriched uranium|closes its airspace|strait of hormuz|visit china|visit russia)\b/.test(qs)) return "world";
  if (/\b(merger|acquisition|bankruptcy|earnings|quarterly|market cap|fed chair|interest rates?)\b/.test(qs)) return "business";
  if (/\b(startup|unicorn|ipo|valuation|venture|funding round|s&p 500|fortune|dow jones|fed rate|rate cut|rate hike)\b/.test(qs)) return "business";
