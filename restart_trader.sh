...
# Start new trader
screen -dmS trader node live/live_trader_node.js --live --loop --interval=300 --position-size=1
sleep 2
...
if screen -ls | grep -q trader; then
    ...
    node live/live_trader_node.js --live --loop --interval=300 --position-size=1 2>&1 | head -20
fi
